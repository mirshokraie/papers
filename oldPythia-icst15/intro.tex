...
Further, the event-driven and highly dynamic nature of \javascript, as well as its runtime interaction with the Document Object Model (DOM) make \javascript applications error-prone \cite{Ocariza:esem2013} and difficult to test. %it challenging to effectively write stable test cases that achieve high coverage. %\karthik{Should we define stable ?} 
...
\item A mutation-based algorithm to effectively generate test oracles;%capable of detecting faults in the \javascript code and the DOM tree of the  application;
