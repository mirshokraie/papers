...
Another threat is that we validate the failed assertions through manual inspection,  which can be error-prone. To mitigate this threat, we carefully inspected the code in which the assertion was failed to make sure that the injected fault was indeed responsible for the assertion failure. %Similar inspection was performed on event-based failed assertions. %after an event is triggered.    
