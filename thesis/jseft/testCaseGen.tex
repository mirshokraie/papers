...
Going back to our running example, one possible event sequence to generate is: \code{\$(`\#cell0').click$\rightarrow$\$(`div \#divElem').click$\rightarrow$\$(`\#startCe\-ll').click}. 
...
\begin{enumerate}
\item The global scope directly (e.g., \code{function f()\{...\}});
\item Variable assignments in the global scope (e.g., \code{var f = function()\-\{...\}});
\item Constructor functions (e.g,  \code{function constructor() \{this.\ me\-mber= function()\{...\}\}});
\item Prototypes (e.g., \code{Con\-structor.prototype.f= function() \{.\-..\}}); 
\end{enumerate}
...
Sets to be covered in our algorithm are $StSet_{K+L}$, where $st_i \in StSet_{K+L}$. We use a common greedy algorithm for obtaining a minimum number of states that can cover all the possible sets (Lines 15-17). 
