...
\begin{enumerate}
\item Revising the function state reduction technique (explained in \secref{jsFuncTest}); The approach diagram (\figref{approach-view}), and the function state reduction algorithm (\algref{stateAbstractionAlgo}) are revised to reflect changes to the state reduction technique. In accordance with the revised approach diagram, we restructured the approach (\secref{approach}) into three sub-sections: (1) maximizing function coverage, (2) extracting DOM and \javascript function states, and (3) mutation analysis.
\end{enumerate}
