...
An intra code dependency is defined as $<initPoint, codeSts>$, where $initPoint$ is the initial point of contact extracted from the intra DOM assertion dependency, and $codeSts$ is the set of statements that are either affected by or influence the $initPoint$. 
\end{mydef}
To find the intra code dependency, we make use of backward as well as forward slicing techniques.
...
Given \code{customer.payable} as the initial slicing criteria in our example, \figref{intraCodeDep} shows the relevant backward slice statements (lines 29, 25, 22, 21, and 16), where object properties \code{customer.payable}, variable \code{price}, \code{coupon.expired}, and properties of the object \code{selItem} are assigned respectively.
