...
\begin{figure}[!t]
  ...
  \mycaption{Finding initial point of contact between DOM-based assertion and the \javascript code.}
  \vspace{-0.1in} 
...
Going back to our running example, the collected execution trace contains information about the mutations of a \code{div} element with class \code{shopContainer}, which pertains to the DOM dependent assertion in \figref{domTest}. Thus, the initial point of contact between the assertion and the relevant DOM mutating statement in the \javascript code is line 37 in function \code{viewCart}.

After inferring DOM mutant statements within the code, we perform backward slice on the retrieved statements to gather the entire set of \javascript statements responsible for mutating a given DOM element.
...
\end{mydef}
\textsc{GetBWSlice} in lines  12 and 18 of \algref{algorithm} computes backward slice with respect to assertion related DOM mutations, and candidate DOM property mutations respectively.
In order to capture code-level dependencies exercised at run-time we use dynamic slicing.
\begin{mydef}[Dynamic Slicing]
...
Once the test case runs, we collect traces from the instrumented code. This trace is used to dynamically extract backward slicing as well as forward slicing statements. Note that in addition to backwards slicing which is later which is later used to generate explicit assertions, we also utilize forwards slicing to generate our implicit assertions (\secref{implicitAssertions}).  
...
Given \code{customer.payable} in line 37 of \figref{example} as the initial slicing criteria, the relevant backward slice statements are lines 29, 27, 25, 21, and 16, where object properties \code{customer.payable}, \code{coupon.expired}, variable \code{price}, and properties of the object \code{selItem} are assigned respectively.
