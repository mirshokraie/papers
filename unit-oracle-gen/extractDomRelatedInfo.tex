...
For each DOM assertion we record the accessed DOM property within the assertion as well as the accessed DOM elements in the test case pertaining to the assertion (\textsc{GetDomAcc} in line 10 of \algref{algorithm}). 
...
In order to link DOM assertions to the relevant \javascript code we further need to track DOM's evolution as the test case runs (\textsc{GetDomMuts} in line 13 of the algorithm). We make use of \code{document.onload} event to log the initial DOM state. An observer module is then used to monitor mutations on the DOM during the test case execution. 
