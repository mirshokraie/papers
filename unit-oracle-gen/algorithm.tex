...
\nl   \If{$\textsc{acc}(prop_{dom}) \geq \frac{1}{\textsc{ReadProperties}(tc_i)}$}{
...
\nl    $impAsstnRel \leftarrow \textsc{GetFWSlice}(expAsstnRel, trace)$\\
      }
     }
...
\nl  $explicitAsstn[func]_{f=1}^{n} \leftarrow \textsc{Accessibles}([func]_{f=1}^{n}, [exppAsstnRel])$\\
...
\nl  $oracles[func]_{f=1}^{n} \leftarrow \{explicitAsstn \cup explicitAsstn \cup candidiateAsstn\}$\\
   }
\nl return $\textsc{Rank}(oracles[func]_{f=1}^{n})$\\
