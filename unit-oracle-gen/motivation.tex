...
Further, by leveraging the execution information obtained from running the DOM-based test case, we can infer potentially important DOM's evolution, which are not checked by the existing DOM dependent assertions. For instance, DOM element with ID \code{couponButt} is accessed several times as the test case runs, however it remains unchecked. Since mutations of \code{couponButt} element pertains to the underlying \javascript code, it is important to assert code statements responsible for mutating the aforementioned DOM element.             
